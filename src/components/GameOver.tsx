...
export default function GameOver({ players, endMessage, onPlayAgain, onMenu }: Props) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="fixed inset-0 bg-[#1a0f06]/95 flex flex-col items-center justify-center p-6 z-50"
    >
      <motion.div
      ...
      >
        ...
        <h2 className="text-3xl font-bold text-[#F5E6C8] font-serif mb-1">
          {isDraw ? 'Égalité !' : `${winner.name} gagne !`}
        </h2>
        ...
        <div className="flex flex-col gap-3 w-full max-w-xs">
          <motion.button
            whileTap={{ scale: 0.97 }}
            onClick={onPlayAgain}
            className="w-full bg-[#C8A96E] text-[#1a0f06] font-bold py-4 rounded-xl text-lg"
          >
            Rejouer
          </motion.button>
          <motion.button
            whileTap={{ scale: 0.97 }}
            onClick={onMenu}
            className="w-full bg-[#2a1a0c] border border-[#8B6914]/40 text-[#C8A96E] font-semibold py-3 rounded-xl"
          >
            Menu
          </motion.button>
        </div>
